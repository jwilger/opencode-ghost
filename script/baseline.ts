...
const main = async () => {
  ...
  const runtime = await readJson(join(root, "tests/runtime/witness.json"))
  const tui_witness = await readJson(join(root, "tests/tui/witness.json"))
  ...
  const claims = {
    ...
    runtime_witness: runtime.case_id,
    tui_witness: tui_witness.case_id,
    required_runtime_routes: runtime.required_routes,
    required_tui_surfaces: tui_witness.required_surfaces,
    required_tui_tests: tui_witness.required_tests,
  }

  for (const id of runtime.required_routes) {
    if (routes.items.some((x: any) => x.operation_id === id)) continue
    throw new Error(`runtime witness route missing from inventory: ${id}`)
  }

  for (const id of tui_witness.required_tui_routes) {
    if (tui.items.some((x: any) => x.id === id)) continue
    throw new Error(`tui witness surface missing from inventory: ${id}`)
  }

  for (const id of tui_witness.required_tests) {
